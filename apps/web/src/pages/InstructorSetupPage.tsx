// InstructorSetupPage - Token-based instructor account creation page at /instructor-setup.
// Reached via the one-time invite link emailed after application approval.
import React, { useEffect, useState } from 'react';
import { useNavigate, useSearchParams } from 'react-router-dom';
import { motion } from 'framer-motion';
import { GraduationCap, Eye, EyeOff, ArrowRight, CheckCircle, AlertCircle } from 'lucide-react';
import apiClient from '../services/api';
import { useAuthStore } from '../store/authStore';

const InstructorSetupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.post('/auth/instructor-setup', {
        token,
      ...
      setDone(true);
      setTimeout(() => navigate('/dashboard/instructor', { replace: true }), 2000);
    } catch (err: any) {
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-[#0F1112] px-4">
        ...
        >
          ...
          <p className="text-gray-600 dark:text-white/70">
            Redirecting you to your instructor dashboard…
          </p>
        </motion.div>
      </div>
    );
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 dark:text-white/60">
            You've been approved! Create a password to activate your account.
          </p>
        </div>
...
export default InstructorSetupPage;
