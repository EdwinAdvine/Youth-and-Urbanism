// PartnerSetupPage - Token-based partner account creation page at /partner-setup.
// Reached via the one-time invite link emailed after partnership application approval.
import React, { useEffect, useState } from 'react';
import { useNavigate, useSearchParams } from 'react-router-dom';
import { motion } from 'framer-motion';
import { Handshake, Eye, EyeOff, ArrowRight, CheckCircle, AlertCircle } from 'lucide-react';
import apiClient from '../services/api';
import { useAuthStore } from '../store/authStore';

const PartnerSetupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.post('/auth/partner-setup', {
        token,
      ...
      setDone(true);
      setTimeout(() => navigate('/dashboard/partner', { replace: true }), 2000);
    } catch (err: any) {
      ...
      >
        ...
        <div className="text-center mb-8">
          <div className="w-14 h-14 bg-[#FF0000]/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Handshake className="w-7 h-7 text-[#FF0000]" />
          </div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
            Set Up Your Partner Account
          </h1>
          <p className="text-sm text-gray-500 dark:text-white/60">
            Your partnership has been approved! Create a password to activate your account.
          </p>
        </div>
