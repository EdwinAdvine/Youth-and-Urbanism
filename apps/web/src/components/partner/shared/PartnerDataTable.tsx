...
interface PartnerDataTableProps<T> {
  data: T[];
...
function PartnerDataTable<T>({
  ...
  return (
    <div className="bg-white dark:bg-[#181C1F] border border-gray-200 dark:border-[#22272B] rounded-xl overflow-hidden">
      {/* Toolbar */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-4 py-3 border-b border-gray-200 dark:border-[#22272B]">
        ...
        <div className="flex items-center gap-2">
          {enableExport && onExport && (
            <div className="relative group">
              <button className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-gray-500 dark:text-white/60 bg-gray-100 dark:bg-[#22272B] rounded-lg hover:text-gray-900 dark:hover:text-white hover:bg-[#2a2f34] transition-colors">
                <Download className="w-3.5 h-3.5" />
                Export
              </button>
              <div className="absolute right-0 top-full mt-1 bg-gray-100 dark:bg-[#22272B] border border-gray-300 dark:border-[#333] rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
                {['csv', 'excel', 'pdf'].map((format) => (
                  <button
                    key={format}
                    onClick={() => onExport(format as 'csv' | 'excel' | 'pdf')}
                    className="block w-full px-4 py-2 text-left text-xs text-gray-600 dark:text-white/70 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5 first:rounded-t-lg last:rounded-b-lg"
                  >
      ...
      {hasSelection && bulkActions && (
        <div className="flex items-center gap-3 px-4 py-2.5 bg-[#E40000]/10 border-b border-[#E40000]/20">
              ...
              <button
                ...
                className={`
                  ...
                  ${action.variant === 'danger'
                    ...
                    : 'text-gray-600 dark:text-white/70 bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10'
                  }
                `}
              >
                {action.icon}
          ...
          <button
            ...
            className="ml-auto text-xs text-gray-400 dark:text-white/40 hover:text-gray-500 dark:hover:text-white/60"
          >
            Clear selection
          </button>
        </div>
        ...
        <table className="w-full" role="table">
          <thead>
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id} className="border-b border-gray-200 dark:border-[#22272B]">
                    ...
                    {header.isPlaceholder ? null : (
                      <button
                        className={`flex items-center gap-1.5 ${
                          header.column.getCanSort() ? 'cursor-pointer hover:text-gray-700 dark:hover:text-white/80' : ''
                        }`}
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={columns.length}>
                  <PartnerEmptyState title={emptyTitle} description={emptyDescription} />
                </td>
              </tr>
            ) : (
              table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <td key={cell.id} className="px-4 py-3 text-sm text-gray-700 dark:text-white/80">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t border-gray-200 dark:border-[#22272B]">
        <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-white/50">
          ...
          <select
            ...
            className="bg-gray-100 dark:bg-[#22272B] border border-gray-300 dark:border-[#333] text-gray-600 dark:text-white/70 rounded px-2 py-1 text-xs focus:outline-none focus:border-[#E40000]/50"
          >
