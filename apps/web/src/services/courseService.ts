...
export const courseService = {
  ...
  async createCourse(courseData: CourseCreate): Promise<Course> {
    const response = await apiClient.post<Course>('/api/v1/courses/', courseData);
  ...
  async getCourseDetails(courseId: string): Promise<CourseWithDetails> {
    const response = await apiClient.get<CourseWithDetails>(`/api/v1/courses/${courseId}`);
    return response.data;
  },
  ...
  async updateCourse(courseId: string, courseData: CourseUpdate): Promise<Course> {
    const response = await apiClient.put<Course>(`/api/v1/courses/${courseId}`, courseData);
    return response.data;
  },
  ...
  async enrollInCourse(courseId: string, paymentId?: string): Promise<Enrollment> {
    const response = await apiClient.post<Enrollment>(`/api/v1/courses/${courseId}/enroll`, null, {
  ...
  async getMyEnrollments(params?: EnrollmentFilterParams): Promise<Enrollment[]> {
    const response = await apiClient.get<Enrollment[]>('/api/v1/courses/my-enrollments', {
  ...
  async completLesson(
    ...
    const response = await apiClient.post<Enrollment>(
      `/api/v1/courses/enrollments/${enrollmentId}/complete-lesson`,
      lessonData
    );
    return response.data;
  },
  ...
  async rateCourse(
    ...
    const response = await apiClient.post<Enrollment>(
      `/api/v1/courses/enrollments/${enrollmentId}/rate`,
      ratingData
    );
    return response.data;
  },
