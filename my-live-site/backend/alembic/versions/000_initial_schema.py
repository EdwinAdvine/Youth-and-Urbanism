...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('admission_number', sa.String(50), unique=True, nullable=False),
        sa.Column('grade_level', sa.String(20), nullable=False),
        sa.Column('enrollment_date', sa.Date(), server_default=sa.func.current_date(), nullable=False),
        sa.Column('is_active', sa.Boolean(), server_default=sa.text('true'), nullable=False),
        sa.Column('learning_profile', postgresql.JSONB(), server_default=sa.text("'{}'::jsonb"), nullable=False),
        sa.Column('competencies', postgresql.JSONB(), server_default=sa.text("'{}'::jsonb"), nullable=False),
        sa.Column('overall_performance', postgresql.JSONB(), server_default=sa.text("'{}'::jsonb"), nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.func.now(), onupdate=sa.func.now()),
    )
