    ...
    <DashboardLayout role="admin">
      ...
      >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 text-sm rounded-lg transition-colors ${
                  hasChanges
                    ? 'bg-[#E40000] text-white hover:bg-[#CC0000]'
                    : 'bg-[#22272B] text-white/30 cursor-not-allowed'
                }`}
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-[#181C1F] border border-[#22272B] rounded-xl p-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-white/60 text-sm">Active API Tokens</span>
              <Lock className="w-5 h-5 text-orange-400" />
            </div>
            <p className="text-2xl font-bold text-white">14</p>
            <p className="text-xs text-white/40 mt-1">Across 4 service integrations</p>
          </div>
        </div>
        ...
        {hasChanges && (
          ...
          >
            ...
            <button
              onClick={handleSave}
              className="ml-auto px-3 py-1.5 text-xs bg-[#E40000] text-white rounded-lg hover:bg-[#CC0000] transition-colors"
            >
              Save Now
            </button>
          </motion.div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#22272B]">
                  <th className="px-6 py-3 text-left text-white/60 font-medium sticky left-0 bg-[#181C1F] z-10 min-w-[200px]">
                    Permission
                  </th>
                  {mockRoles.map((role) => (
                  ...
                  <React.Fragment key={category}>
                    ...
                    <tr className="bg-[#0F1112]">
                      <td
                        ...
                        className="px-6 py-2 text-xs font-bold text-white/40 uppercase tracking-wider"
                      >
                        {category}
                      </td>
                    </tr>
                    {perms.map((perm) => (
                      ...
                      >
                        <td className="px-6 py-3 sticky left-0 bg-[#181C1F] z-10">
                          <div>
                            <span className="text-white/80 text-sm">{perm.display_name}</span>
                            <p className="text-[11px] text-white/30 mt-0.5">{perm.description}</p>
                          </div>
                        </td>
                        {mockRoles.map((role) => (
                            ...
                            <button
                              ...
                              className={`w-5 h-5 rounded border-2 inline-flex items-center justify-center transition-all ${
                                matrix[role.id]?.[perm.id]
                                  ...
                                  : 'border-[#444] hover:border-white/40 cursor-pointer'
                              }`}
                              title={
                                role.id === 'admin'
                                  ? 'Admin permissions are locked'
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-[#22272B] text-white/50">
                    {role.user_count} users
                  </span>
                </div>
                <p className="text-xs text-white/40 mb-3">{role.description}</p>
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-1.5 rounded-full bg-[#22272B] overflow-hidden">
                    <div
