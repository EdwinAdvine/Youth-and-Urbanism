      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-[#181C1F] rounded-xl border border-gray-200 dark:border-[#22272B] p-4">
          ...
          <div className="flex items-baseline gap-1 mt-1">
            ...
            <p className="text-sm text-gray-400 dark:text-white/30">/5.0</p>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-[#22272B]">
              {filteredTickets.map((ticket) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs ${ticket.assignedTo === 'Unassigned' ? 'text-red-400' : 'text-gray-500 dark:text-white/60'}`}>
                      {ticket.assignedTo}
                    </span>
                  </td>
                  <td className="px-4 py-3">{getSLAIndicator(ticket.slaStatus, ticket.slaRemaining)}</td>
                  <td className="px-4 py-3">
                    <span className="text-xs text-gray-500 dark:text-white/50">{formatDate(ticket.createdAt)}</span>
                  </td>
                </tr>
              ))}
            </tbody>
