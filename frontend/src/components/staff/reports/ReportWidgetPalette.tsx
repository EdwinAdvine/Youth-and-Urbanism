...
const ReportWidgetPalette: React.FC<ReportWidgetPaletteProps> = ({ onAddWidget }) => {
  return (
    <div className="bg-[#181C1F] border border-[#22272B] rounded-xl p-4">
      <h3 className="text-sm font-semibold text-white mb-3">Widget Palette</h3>
        ...
        {WIDGET_TEMPLATES.map((template) => {
          ...
          return (
            <button
              ...
              className="w-full flex items-start gap-3 p-3 rounded-lg bg-[#22272B]/50 hover:bg-[#22272B] border border-transparent hover:border-[#E40000]/30 transition-all text-left"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-white/40 mt-0.5">{template.description}</p>
              </div>
            </button>
          );
        })}
