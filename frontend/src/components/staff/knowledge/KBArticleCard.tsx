...
const KBArticleCard: React.FC<KBArticleCardProps> = ({ article, onClick, relevanceScore }) => {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full p-4 bg-[#181C1F] border border-[#22272B] rounded-xl hover:border-[#E40000]/30 transition-all text-left group"
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-sm font-semibold text-white group-hover:text-[#FF4444] transition-colors truncate">
              {article.title}
            </h3>
          </div>
          <p className="text-xs text-white/50 line-clamp-2">{article.excerpt}</p>
        </div>
      ...
      <div className="flex items-center gap-3 mt-3 text-[10px] text-white/40">
        <span
          className={`px-2 py-0.5 rounded-full ${
            categoryColors[article.category] || 'bg-[#22272B] text-white/40'
          }`}
        >
          {article.category}
        </span>
        ...
        <div className="flex items-center gap-1.5 mt-2">
          ...
          <div className="flex flex-wrap gap-1">
            ...
            {article.tags.length > 3 && (
              <span className="text-[10px] text-white/30">+{article.tags.length - 3}</span>
            )}
          </div>
        </div>
      )}
      ...
      {/* Stats */}
      <div className="flex items-center gap-4 mt-3 pt-3 border-t border-[#22272B]">
