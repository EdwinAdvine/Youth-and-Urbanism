    ...
    <div className="bg-[#181C1F] border border-[#22272B] rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#22272B]">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-white">Adaptive Paths</h3>
        </div>
        <button
      ...
      <div className="p-5">
        {paths.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-white/40">No adaptive paths configured</p>
            ...
            {paths.map((path) => (
              <div
                ...
                className={`relative p-4 rounded-lg border transition-colors ${
                  hoveredPath === path.id
                    ? 'border-[#333] bg-[#22272B]/50'
                    : 'border-[#22272B] bg-[#22272B]/30'
                }`}
              >
                {/* Remove button */}
                <button
                  onClick={() => handleRemovePath(path.id)}
                  className={`absolute top-3 right-3 text-white/20 hover:text-red-400 transition-all ${
                ...
                <div className="mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <select
                      ...
                      className="px-3 py-1.5 bg-[#181C1F] border border-[#22272B] rounded-lg text-sm text-white outline-none focus:border-[#E40000]/50 appearance-none cursor-pointer"
                    >
                      {[1, 2, 3, 4, 5].map((d) => (
                        <option key={d} value={d} className="bg-[#22272B]">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="p-3 rounded-lg bg-green-500/5 border border-green-500/20">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-white/40">Go to:</span>
                      <select
                        value={path.correctTarget}
                        onChange={(e) =>
                          handleUpdatePath(path.id, 'correctTarget', Number(e.target.value))
                        }
                        className="px-2 py-1 bg-[#181C1F] border border-[#22272B] rounded text-xs text-white outline-none focus:border-green-500/50 appearance-none cursor-pointer"
                      >
                        {[1, 2, 3, 4, 5].map((d) => (
                          <option key={d} value={d} className="bg-[#22272B]">
                            {d} - {DIFFICULTY_LABELS[d]}
                  ...
                  <div className="p-3 rounded-lg bg-red-500/5 border border-red-500/20">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        className="px-2 py-1 bg-[#181C1F] border border-[#22272B] rounded text-xs text-white outline-none focus:border-red-500/50 appearance-none cursor-pointer"
                      >
                        {[1, 2, 3, 4, 5].map((d) => (
                          <option key={d} value={d} className="bg-[#22272B]">
                            {d} - {DIFFICULTY_LABELS[d]}
