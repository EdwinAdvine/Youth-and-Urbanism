  ...
  return (
    <div className="bg-[#181C1F] border border-[#22272B] rounded-xl p-5">
      ...
      <div className="space-y-2">
        {pending.length === 0 ? (
          <p className="text-sm text-white/40 text-center py-4">All tasks completed!</p>
        ) : (
          pending.slice(0, 5).map((task) => (
            <div
              key={task.id}
              className="flex items-center gap-3 p-2.5 rounded-lg bg-[#22272B]/50"
            >
              <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${task.isOverdue ? 'bg-red-400' : 'bg-green-400'}`} />
              <div className="min-w-0 flex-1">
                <p className="text-sm text-white truncate">{task.title}</p>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className={`text-[10px] ${typeColors[task.type] || 'text-white/40'}`}>{task.type}</span>
                  <span className={`text-[10px] flex items-center gap-0.5 ${task.isOverdue ? 'text-red-400' : 'text-white/40'}`}>
      ...
      {completed.length > 0 && (
        <div className="mt-3 pt-3 border-t border-[#22272B]">
