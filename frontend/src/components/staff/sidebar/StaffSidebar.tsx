...
const StaffSidebar: React.FC<StaffSidebarProps> = ({ isOpen, onClose }) => {
  ...
  const { counters, openSidebarSections, toggleSidebarSection, globalSearch, setGlobalSearch, viewMode, setViewMode } = useStaffStore();
        ...
        <div className="p-3 border-b border-[#22272B]">
          ...
          <button
            ...
            className="mt-2 w-full flex items-center justify-between px-3 py-1.5 text-xs font-medium rounded-md
              bg-[#22272B] border border-[#333] text-white/60 hover:text-white/80 hover:border-[#444] transition-colors"
          >
            <div className="flex items-center gap-1.5">
              <ToggleLeft className="w-3.5 h-3.5" />
              <span>{viewModeLabel[viewMode]}</span>
            </div>
            <span className="text-white/30 text-[10px]">Click to switch</span>
          </button>
        </div>
